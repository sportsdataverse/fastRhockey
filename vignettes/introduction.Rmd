...
```{r install_fastRhockey_cran, message=FALSE,eval=FALSE}
install.packages("fastRhockey")
```

If you would prefer the `devtools` installation:

```{r install_fastRhockey_devtools, message=FALSE,eval=FALSE}
...
If you look with `fastRhockey::`, there are more functions available, but those are helper functions to pull raw data (`phf_game_raw`) and then to process the raw data into a usable format (`helper_phf_*`). 
