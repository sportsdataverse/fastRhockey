#' @title **NHL Edge Skater Speed Top 10**
#' @description Returns the NHL Edge top-10 "fastest skaters" leaderboard
...
#' @examples
#' \donttest{
#'   try(nhl_edge_skater_speed_top_10(
#'       positions = "F",
#'       sort_by = "total"
#'   ))
#' }
nhl_edge_skater_speed_top_10 <- function(
    positions,
    sort_by,
    season = NULL,
    game_type = 2
    ...
    raw <- .nhl_edge_api(
        base = glue::glue("skater-speed-top-10/{positions}/{sort_by}"),
        season = season,
        game_type = game_type,
        prefix = "v1/edge"
    )
    ...
    if (is.null(df) || (is.data.frame(df) && nrow(df) == 0)) {
        message(glue::glue(
            "{Sys.time()}: No NHL Edge skating-speed top-10 data returned for {positions}/{sort_by}"
        ))
        return(NULL)
    }

    df <- janitor::clean_names(dplyr::as_tibble(df))
    make_fastRhockey_data(df, "NHL Edge Skater Speed Top 10", Sys.time())
}
