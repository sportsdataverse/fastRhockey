#' @title **NHL Edge Skater Skating Speed Detail**
...
#' }
nhl_edge_skater_skating_speed_detail <- function(
    player_id,
    season = NULL,
    game_type = 2
) {
    raw <- .nhl_edge_api(
        base = glue::glue("skater-skating-speed-detail/{player_id}"),
        season = season,
        game_type = game_type,
        prefix = "v1/edge"
    )
    ...
    if (is.null(df) || (is.data.frame(df) && nrow(df) == 0)) {
        message(glue::glue(
            "{Sys.time()}: No NHL Edge skating-speed detail returned for player_id={player_id}"
        ))
        return(NULL)
    }
