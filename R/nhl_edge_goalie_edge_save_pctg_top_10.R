#' @title **NHL Edge Goalie Edge Save Percentage Top 10**
#' @description Returns the NHL Edge top-10 goalie save-percentage
...
#' }
nhl_edge_goalie_edge_save_pctg_top_10 <- function(
    sort_by,
    season = NULL,
    game_type = 2
) {
    raw <- .nhl_edge_api(
        base = glue::glue("goalie-edge-save-pctg-top-10/{sort_by}"),
        season = season,
        game_type = game_type,
        prefix = "v1/edge"
    )
    ...
    if (is.null(df) || (is.data.frame(df) && nrow(df) == 0)) {
        message(glue::glue(
            "{Sys.time()}: No NHL Edge goalie edge save-pctg top-10 data returned for {sort_by}"
        ))
        return(NULL)
    }
