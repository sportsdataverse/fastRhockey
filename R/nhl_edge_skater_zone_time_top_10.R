#' @title **NHL Edge Skater Zone Time Top 10**
#' @description Returns the NHL Edge top-10 zone-time leaderboard for a
...
#'   ))
#' }
nhl_edge_skater_zone_time_top_10 <- function(
    positions,
    strength,
    sort_by,
    season = NULL,
    ...
    raw <- .nhl_edge_api(
        base = glue::glue(
            "skater-zone-time-top-10/{positions}/{strength}/{sort_by}"
        ),
        season = season,
        game_type = game_type,
        prefix = "v1/edge"
    ...
    if (is.null(df) || (is.data.frame(df) && nrow(df) == 0)) {
        message(glue::glue(
            "{Sys.time()}: No NHL Edge zone-time top-10 data returned for {positions}/{strength}/{sort_by}"
        ))
        return(NULL)
    }
