#' @title **NHL Edge Skater Shot Location Detail**
...
#'   (default), 3 = playoffs.
#' @return A `fastRhockey_data` tibble of shot locations, or `NULL` on
...
#' }
nhl_edge_skater_shot_location_detail <- function(
    player_id,
    season = NULL,
    game_type = 2
) {
    raw <- .nhl_edge_api(
        base = glue::glue("skater-shot-location-detail/{player_id}"),
        season = season,
        game_type = game_type,
        prefix = "v1/edge"
    )
    ...
    if (is.null(df) || (is.data.frame(df) && nrow(df) == 0)) {
        message(glue::glue(
            "{Sys.time()}: No NHL Edge shot-location detail returned for player_id={player_id}"
        ))
        return(NULL)
    }
