#' @title **NHL Edge Goalie Detail**
#' @description Returns the NHL Edge advanced-metrics detail payload for a
#'   single goalie. Wraps
...
#'   (default), 3 = playoffs.
#' @return A `fastRhockey_data` tibble of advanced metrics, or `NULL` on
...
#' @examples
#' \donttest{
#'   try(nhl_edge_goalie_detail(player_id = 8475883))
#' }
nhl_edge_goalie_detail <- function(
    player_id,
    season = NULL,
    game_type = 2
) {
    ...
    if (is.null(df) || (is.data.frame(df) && nrow(df) == 0)) {
        message(glue::glue(
            "{Sys.time()}: No NHL Edge goalie detail returned for player_id={player_id}"
        ))
        return(NULL)
    }

    df <- janitor::clean_names(dplyr::as_tibble(df))
    make_fastRhockey_data(df, "NHL Edge Goalie Detail", Sys.time())
}
