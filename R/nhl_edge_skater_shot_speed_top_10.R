#' @title **NHL Edge Skater Shot Speed Top 10**
#' @description Returns the NHL Edge top-10 "hardest-shot" leaderboard for
...
#'   (e.g., `"F"` for forwards, `"D"` for defensemen, or a combined
#'   filter).
...
#'   ))
#' }
nhl_edge_skater_shot_speed_top_10 <- function(
    positions,
    sort_by,
    season = NULL,
    game_type = 2
    ...
    raw <- .nhl_edge_api(
        base = glue::glue("skater-shot-speed-top-10/{positions}/{sort_by}"),
        season = season,
        game_type = game_type,
        prefix = "v1/edge"
    )
    ...
    if (is.null(df) || (is.data.frame(df) && nrow(df) == 0)) {
        message(glue::glue(
            "{Sys.time()}: No NHL Edge shot-speed top-10 data returned for {positions}/{sort_by}"
        ))
        return(NULL)
    }
