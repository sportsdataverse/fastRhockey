#' @title **NHL Edge Skater Comparison**
#' @description Returns the NHL Edge skater comparison payload for a single
...
#'   (default), 3 = playoffs.
#' @return A `fastRhockey_data` tibble comparing the focus player against
...
#' }
nhl_edge_skater_comparison <- function(
    player_id,
    season = NULL,
    game_type = 2
) {
    raw <- .nhl_edge_api(
        base = glue::glue("skater-comparison/{player_id}"),
        season = season,
        game_type = game_type,
        prefix = "v1/edge"
    )
    ...
    if (is.null(df) || (is.data.frame(df) && nrow(df) == 0)) {
        message(glue::glue(
            "{Sys.time()}: No NHL Edge skater comparison data returned for player_id={player_id}"
        ))
        return(NULL)
    }

    df <- janitor::clean_names(dplyr::as_tibble(df))
    make_fastRhockey_data(df, "NHL Edge Skater Comparison", Sys.time())
}
