...
``` r
espn_nhl_calendar_offseason(season = most_recent_nhl_season(), ...)
```
...
A `fastRhockey_data` tibble with one row per offseason date. May return
zero rows when the offseason calendar is empty for the requested season.
