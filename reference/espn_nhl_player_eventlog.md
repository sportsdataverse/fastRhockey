...
``` r
espn_nhl_player_eventlog(athlete_id, ...)
```
...
A `fastRhockey_data` tibble with one row per event (game) in the current
season. Use `statistics_ref` to retrieve per-game statistics.
...
#> ℹ Data updated: 2026-06-12 22:23:03 UTC
#> # A tibble: 18 × 9
