...
ids <- c("Snap shot", "Empty Net", "Even Strength", 
         "Tie", "Power Play", "Wrist shot", "Minor", "Holding", "giveaway", "Tripping",
         "Roughing", "Hooking", "Interference", "Slashing", "blocked",
         "faceoff won", "saved", "Pulled goalie", "Returned goalie", "Starting goalie")
##################################################
#### updated tags
away <- "[:digit:] GvA|[:digit:] TkA|[:digit:] Blk"
fill <- "from|by|against|to|and|giveaway"
goalie <- "Starting goalie|Pulled goalie|Returned goalie"
fo <- "faceoff won"
ice <- "Even Strength|Empty Net|Power Play"
shots <- "Snap shot|Wrist shot"
res <- "blocked|saved"
pen <- "Holding|Tripping|Roughing|Hooking|Interference|Slashing"
type <- "Minor|Major"
...
pbp_data <- function(data) {
  ...
  pbp <- pbp %>%
    ...
    mutate(on_ice_situation = ifelse((sec_from_start >= start_power_play & 
                                       ...
                                       (on_ice_situation == "Power Play"), "Power Play", on_ice_situation),
