...
All HTTP requests flow through an internal `httr2` helper
...
(`list(url=, port=, username=, password=, auth=)`). The
session-[`options()`](https://rdrr.io/r/base/options.html) form is
