test_that("NHL - Edge Skater Shot Location Top 10 (current)", {
    skip_on_cran()
    skip_nhl_test()
    x <- nhl_edge_skater_shot_location_top_10(
        position = "F",
        category = "high",
        sort_by = "total"
    )
...
test_that("NHL - Edge Skater Shot Location Top 10 (specific season)", {
    skip_on_cran()
    skip_nhl_test()
    x <- nhl_edge_skater_shot_location_top_10(
        position = "F",
