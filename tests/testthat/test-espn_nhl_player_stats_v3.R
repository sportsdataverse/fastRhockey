test_that("ESPN NHL Player Stats V3 returns valid wide tibble", {
  skip_on_cran()
  skip_espn_test()
  ...
  x <- espn_nhl_player_stats_v3(athlete_id = athlete_id, season = 2025)

  if (!is.data.frame(x) || nrow(x) == 0) {
    skip("No rows returned from espn_nhl_player_stats_v3 at test time")
  }
