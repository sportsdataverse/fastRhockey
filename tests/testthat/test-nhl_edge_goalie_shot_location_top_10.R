test_that("NHL - Edge Goalie Shot Location Top 10 (current)", {
    skip_on_cran()
    skip_nhl_test()
    x <- nhl_edge_goalie_shot_location_top_10(
        category = "high",
        sort_by = "savePctg"
    )
...
test_that("NHL - Edge Goalie Shot Location Top 10 (specific season)", {
    skip_on_cran()
    skip_nhl_test()
    x <- nhl_edge_goalie_shot_location_top_10(
        category = "high",
