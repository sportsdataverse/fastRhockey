...
* **Deprecated:** nine NHL Edge "top-10" leaderboard wrappers whose upstream
  endpoints the NHL has removed (every request returns HTTP 404):
  ...
  are unaffected.
* **Fix:** `pwhl_playoff_bracket()` errored with `$ operator is invalid for
  atomic vectors` and returned `NULL`. The HockeyTech feed returns
...
* **Fix:** `nhl_schedule(include_data_flags = TRUE)` failed with
  ...
  the 16 data-availability flag columns attach correctly.
* **Fix:** NHL Edge "top-10" leaderboard endpoints the NHL has removed return
  HTTP 404. The Edge fetch helper now passes `terminate_on` to `httr::RETRY`
  ...
  loop and log noise. Transient 5xx/timeouts are still retried.
* **Test infrastructure:** `test-load_nhl.R` and `test-calculate_xg.R` no
...
* Every working exported function (165 across the NHL, NHL Edge, NHL Records,
  PWHL, and loader families) now documents its return value with a
  `col_name | types | description` table, matching the convention used in
  ...
  from live API responses, so the tables reflect the actual returned schema.
