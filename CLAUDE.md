...
| `load_nhl_officials()` | Loader | nhl_officials |
| `load_nhl_shots_by_period()` | Loader | nhl_shots_by_period |
| `load_nhl_shootout()` | Loader | nhl_shootout |
| `update_nhl_db()` | DB | (uses load_nhl_pbp) |

All `load_nhl_*()` helpers share `.nhl_release_loader()` (in `R/nhl_loaders.R`),
...
`fastRhockey_data` S3 class. The DB helpers (`update_nhl_db()`,
`build_nhl_db()`, `get_missing_nhl_games()`) live in the same file.

`nhl_schedule()` exposes an opt-in `include_data_flags = FALSE` parameter
that, when set, left-joins the cached `nhl_games_in_data_repo` index from
the data repo and adds one logical column per pre-compiled dataset
(`PBP`, `team_box`, `player_box`, `skater_box`, `goalie_box`, `game_info`,
